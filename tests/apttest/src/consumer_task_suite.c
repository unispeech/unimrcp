...
static apt_bool_t task_on_start_complete(apt_composite_task_t *task)
{
	apt_log(APT_PRIO_INFO,"On Task Start");
	return TRUE;
}

static apt_bool_t task_on_terminate_complete(apt_composite_task_t *task)
{
	apt_log(APT_PRIO_INFO,"On Task Terminate");
	return TRUE;
}

static apt_bool_t task_msg_process(apt_composite_task_t *task, apt_task_msg_t *msg)
{
...
static apt_bool_t consumer_task_test_run(apt_test_suite_t *suite, int argc, const char * const *argv)
{
	...
	apt_composite_task_t *composite_task;
	apt_task_t *task;
	...
	cvtable.process_msg = task_msg_process;
	cvtable.on_start_complete = task_on_start_complete;
	cvtable.on_terminate_complete = task_on_terminate_complete;
	...
	consumer_task = apt_consumer_task_create(NULL,&vtable,&cvtable,msg_pool,suite->pool);
	if(!consumer_task) {
		apt_log(APT_PRIO_WARNING,"Failed to Create Consumer Task");
		return FALSE;
	}

	composite_task = apt_consumer_task_base_get(consumer_task);
	task = apt_composite_task_base_get(composite_task);
